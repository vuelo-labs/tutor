...
// ============================================
const SKILLS = [
  { id: "clarity", name: "Clarity", short: "Vague thought → precise question", icon: "◎", color: "#3B82F6" },
  { id: "decomposition", name: "Decomposition", short: "Whole → meaningful parts", icon: "◫", color: "#8B5CF6" },
  { id: "relationships", name: "Relationships", short: "Parts → connected system", icon: "⬡", color: "#EC4899" },
  { id: "outcomes", name: "Outcome Definition", short: "Vague goal → measurable finish line", icon: "◉", color: "#F59E0B" },
  { id: "gaps", name: "Gap Analysis", short: "Here → there, what's missing", icon: "▧", color: "#10B981" },
  { id: "procedural", name: "Procedural Thinking", short: "Intent → executable steps", icon: "▤", color: "#06B6D4" },
...
const ASSESSMENT_SCENARIOS = [
  ...
  },
  {
    ...
    rubric: {
      ...
      high: ["feedback loops", "competitor response → price war risk", "customer anchoring → future pricing power reduced"],
    },
  },
  {
    skill: "outcomes",
    ...
    rubric: {
      ...
      high: ["deliver 20-min talk to 50+ people, Q&A handled, audience NPS 8+, recordable quality, within 6 months"],
    },
  },
  {
    skill: "gaps",
    ...
    rubric: {
      ...
      high: ["categorised as: validate now / build / automate / defer", "sequenced by dependency", "risk-weighted"],
    },
  },
  {
    skill: "procedural",
    ...
    rubric: {
      ...
      high: ["input: invitation details. step 1: does it have an agenda? no → decline with template..."],
    },
  },
  {
    skill: "evaluation",
    ...
    rubric: {
      ...
      high: ["TAM ≠ SAM ≠ SOM", "growth rate without base is meaningless", "no competitive analysis", "no mention of localisation cost"],
    },
  },
];
...
const buildAssessmentPrompt = (scenario, skillName) =>
  `You are an assessment engine for a structured thinking course. You are evaluating a user's capability in "${skillName}".
...
const buildTrainingPrompt = (skill, level, userProfile) => {
  ...
  const intensity =
    level <= 3 ? "supportive and guiding" : level <= 6 ? "challenging and direct" : "rigorous, Socratic, expects excellence";
...
// ============================================
// STORAGE — using localStorage (built into every browser, no backend needed yet)
// Key insight: localStorage persists across page refreshes but not across devices.
// We'll upgrade this to a real database (Supabase) in a later step so profiles
// sync across devices and we can do proactive delivery.
// ============================================
...
// ============================================
// API CALL — now goes through our secure proxy, not directly to Anthropic
// The browser calls /api/claude (our Netlify Function) which holds the API key
// ============================================
...
const SkillRadar = ({ skills: skillLevels }) => {
  ...
  const axisLines = SKILLS.map((_, i) => {
    const a = (Math.PI * 2 * i) / SKILLS.length - Math.PI / 2;
    return <line key={i} x1={cx} y1={cy} x2={cx + maxR * Math.cos(a)} y2={cy + maxR * Math.sin(a)} stroke="#1E293B" strokeWidth={0.5} />;
  });
    ...
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      {points.map((p, i) => (
        <g key={i}>
          ...
          <text x={p.labelX} y={p.labelY} textAnchor="middle" dominantBaseline="middle" fill="#94A3B8" fontSize={9} fontFamily="monospace">
...
const SkillBar = ({ skill, level, onClick }) => (
  ...
  >
    ...
    <div style={{ fontFamily: "monospace", fontSize: 14, fontWeight: 700, color: skill.color, minWidth: 36, textAlign: "right" }}>
...
const Dots = () => {
  ...
  return (
    <div style={{ display: "flex", justifyContent: "flex-start", marginBottom: 10 }}>
      <div style={{ padding: "12px 16px", borderRadius: "14px 14px 14px 4px", background: "#1E293B", color: "#475569", fontSize: 14, fontFamily: "monospace" }}>
        thinking{d}
      </div>
    </div>
  );
...
export default function ThinkFirstEngine() {
  ...
  const handleAssessmentResponse = async () => {
    ...
    const result = await callAI(buildAssessmentPrompt(scenario, skill.name), [{ role: "user", content: userMsg }]);
    ...
    if (nextIdx < ASSESSMENT_SCENARIOS.length) {
      setMessages([...newMsgs,
        ...
        { role: "assistant", content: ASSESSMENT_SCENARIOS[nextIdx].prompt },
      ]);
      setAssessmentIndex(nextIdx);
    } else {
      // Assessment complete — build and save profile
      const skillLevels = {};
      const skillDetails = {};
      ...
      setMessages([...newMsgs, { role: "assistant", content: `${parsed.assessment}\n\nAssessment complete. Building your profile...` }]);
  ...
  const startTraining = async (skill) => {
    ...
    const result = await callAI(buildTrainingPrompt(skill, level, context), [{ role: "user", content: "Start a training exercise." }]);
  ...
  const handleTrainingResponse = async () => {
    ...
    const result = await callAI(buildTrainingPrompt(activeSkill, level, context), newMsgs.map((m) => ({ role: m.role, content: m.content })));
  ...
  const completeSession = () => {
    ...
    updated.history = [...(updated.history || []), { skill: activeSkill.id, level: currentLevel, date: new Date().toISOString(), exchanges: messages.length }];
    saveProfile(updated);
    setProfile(updated);
    setActiveSkill(null);
    setScreen("dashboard");
  ...
  const getRecommendedSkill = () => {
    ...
    return SKILLS.reduce((min, s) => (profile.skills[s.id] || 0) < (profile.skills[min.id] || 0) ? s : min, SKILLS[0]);
  };
  ...
  const styles = {
    app: { height: "100vh", background: "#0B1121", color: "#E2E8F0", fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" },
  };
  ...
  if (screen === "loading") {
    return (
      <div style={{ ...styles.app, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: "#475569", fontFamily: "monospace", fontSize: 14 }}>loading...</div>
      </div>
    );
  }
  ...
  if (screen === "welcome") {
    return (
      <div style={{ ...styles.app, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ textAlign: "center", maxWidth: 540 }}>
          <div style={{ fontFamily: "monospace", fontSize: 11, letterSpacing: 4, color: "#475569", textTransform: "uppercase", marginBottom: 24 }}>
            Cognitive Infrastructure Engine
          </div>
          <h1 style={{ fontSize: 64, fontWeight: 700, margin: "0 0 8px", letterSpacing: -2, lineHeight: 1 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center", marginBottom: 40 }}>
            {SKILLS.map((s) => (
              <div key={s.id} style={{ padding: "6px 14px", borderRadius: 6, fontSize: 12, border: `1px solid ${s.color}33`, color: s.color, fontFamily: "monospace" }}>
                {s.icon} {s.name}
              </div>
            ))}
          </div>
          <button
            ...
            style={{ padding: "16px 48px", borderRadius: 12, border: "none", background: "#3B82F6", color: "#FFF", fontSize: 16, fontWeight: 600, cursor: "pointer", transition: "all 0.2s" }}
            onMouseEnter={(e) => (e.currentTarget.style.transform = "translateY(-2px)")}
            onMouseLeave={(e) => (e.currentTarget.style.transform = "translateY(0)")}
          >
            Begin Assessment →
          </button>
          <div style={{ fontSize: 12, color: "#334155", marginTop: 16, fontFamily: "monospace" }}>
  ...
  if (screen === "assessment" || screen === "training") {
    ...
    return (
      <div style={{ ...styles.app, display: "flex", flexDirection: "column" }}>
        {/* Header */}
        <div style={{ padding: "14px 24px", borderBottom: "1px solid #1E293B", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
          <div style={{ fontFamily: "monospace", fontSize: 12, color: headerColor, letterSpacing: 1.5, textTransform: "uppercase" }}>
            {headerText}
          </div>
          {!isAssessment && (
            <button onClick={completeSession} style={{ padding: "6px 16px", borderRadius: 8, border: `1px solid ${headerColor}`, background: "transparent", color: headerColor, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
              Complete Session →
            </button>
          )}
          {isAssessment && (
            <div style={{ display: "flex", gap: 4 }}>
              {ASSESSMENT_SCENARIOS.map((_, i) => (
                <div key={i} style={{ width: 8, height: 8, borderRadius: "50%", background: i < assessmentIndex ? "#3B82F6" : i === assessmentIndex ? "#60A5FA" : "#1E293B", transition: "all 0.3s" }} />
              ))}
            </div>
          )}
        </div>
        ...
        {/* Input */}
        <div style={{ padding: "14px 24px 18px", borderTop: "1px solid #1E293B", flexShrink: 0 }}>
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button
              ...
              style={{ padding: "12px 20px", borderRadius: 10, border: "none", background: loading || !input.trim() ? "#1E293B" : headerColor, color: "#FFF", fontSize: 14, fontWeight: 600, cursor: loading || !input.trim() ? "not-allowed" : "pointer" }}
            >
              →
            </button>
          </div>
  ...
  if (screen === "dashboard" && profile) {
    ...
    return (
      <div style={{ ...styles.app, display: "flex", overflow: "hidden" }}>
        {/* Left sidebar */}
        <div style={{ width: 340, borderRight: "1px solid #1E293B", display: "flex", flexDirection: "column", flexShrink: 0, overflow: "hidden", background: "#0D1526" }}>
          <div style={{ padding: "24px 20px 16px" }}>
            <div style={{ fontFamily: "monospace", fontSize: 11, letterSpacing: 3, color: "#475569", textTransform: "uppercase", marginBottom: 8 }}>
              THINK<span style={{ color: "#3B82F6" }}>FIRST</span>
            </div>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "8px 16px 16px" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {SKILLS.map((s) => <SkillBar key={s.id} skill={s} level={profile.skills[s.id] || 0} onClick={() => startTraining(s)} />)}
            </div>
          </div>

          <div style={{ padding: "12px 16px", borderTop: "1px solid #1E293B", flexShrink: 0 }}>
            <button onClick={resetProfile} style={{ width: "100%", padding: "8px", borderRadius: 8, border: "1px solid #1E293B", background: "transparent", color: "#475569", fontSize: 11, cursor: "pointer", fontFamily: "monospace" }}>
              Reset Profile
            </button>
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "32px 40px" }}>
          <h2 style={{ margin: "0 0 8px", fontSize: 28, fontWeight: 700, letterSpacing: -1 }}>Your Cognitive Profile</h2>
          <p style={{ color: "#64748B", fontSize: 14, margin: "0 0 32px" }}>Tap any skill on the left to start a training session.</p>

          {/* Recommended skill */}
          <div
            onClick={() => startTraining(recommended)}
            style={{ padding: "24px 28px", borderRadius: 14, border: `1px solid ${recommended.color}33`, background: `linear-gradient(135deg, ${recommended.color}08, ${recommended.color}03)`, cursor: "pointer", marginBottom: 32, transition: "all 0.2s" }}
          ...
          >
            <div style={{ fontFamily: "monospace", fontSize: 10, letterSpacing: 2, color: recommended.color, textTransform: "uppercase", marginBottom: 10 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div>
                <div style={{ fontSize: 18, fontWeight: 700 }}>
                  {recommended.name}
                  <span style={{ fontFamily: "monospace", fontSize: 12, color: recommended.color, marginLeft: 10 }}>
          ...
          {/* Skill detail grid */}
          <div style={{ fontFamily: "monospace", fontSize: 10, letterSpacing: 2, color: "#475569", textTransform: "uppercase", marginBottom: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {SKILLS.map((s) => {
              ...
              return (
                <div key={s.id} style={{ padding: "16px 18px", borderRadius: 10, border: "1px solid #1E293B", background: "#0F172A" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                    <div style={{ fontSize: 14, fontWeight: 600 }}>
                      <span style={{ color: s.color, marginRight: 8 }}>{s.icon}</span>{s.name}
                    </div>
                    <div style={{ padding: "2px 10px", borderRadius: 6, background: `${s.color}22`, color: s.color, fontFamily: "monospace", fontSize: 11, fontWeight: 700 }}>
                      L{level} {levelData?.name}
                    </div>
                  </div>
                  {detail ? (
                    <>
                      <div style={{ fontSize: 12, color: "#94A3B8", marginBottom: 6 }}><span style={{ color: "#10B981" }}>+</span> {detail.strength}</div>
                      <div style={{ fontSize: 12, color: "#94A3B8" }}><span style={{ color: "#F59E0B" }}>→</span> {detail.gap}</div>
                    </>
                  ) : (
            ...
            <div style={{ marginTop: 32 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {profile.history.slice(-10).reverse().map((h, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 14px", borderRadius: 8, border: "1px solid #1E293B", fontSize: 12 }}>
                      <span style={{ color: sk?.color }}>{sk?.icon}</span>
                      <span style={{ color: "#94A3B8", flex: 1 }}>{sk?.name} · L{h.level}</span>
                      <span style={{ color: "#475569", fontFamily: "monospace", fontSize: 11 }}>{new Date(h.date).toLocaleDateString()}</span>
                    </div>
                  );
                })}
              </div>
